...
# This script crate huge amount of udp connections to test
# connection  tracking subsystem in linux kernel.
# The sigle iperf3 udp stream can reflect conntrack hash matching speed.
...
cleanup()
{
	...
	sysctl net.core.rmem_default=212992 > /dev/null
...
enable_conntrack()
{
	...
	for n in S C F
	do
		ip netns exec $n sysctl -w net.netfilter.nf_conntrack_udp_timeout=50000
		ip netns exec $n sysctl -w net.netfilter.nf_conntrack_udp_timeout_stream=100000
	done
}
...
#	ip netns exec C iperf3 --client 10.0.1.99 --udp --bitrate 0 --pacing-timer 1 --time 0 --length 64 --parallel 1 --version4 --udp-counters-64bit &
#	sleep 3

	# IPv4
	# 4×500×5×500 = 5000000 entries, could eat nearly 40Gi RAM.
	# 1 thread
#	ip netns exec S ./server -u -H 10.0.1.100,10.0.1.101 -P 1001-1500 &
#	ip netns exec C ./client -u -H 10.0.1.100,10.0.1.101 -P 1001-1500 -h 10.0.2.101,10.0.2.102,10.0.2.103,10.0.2.104,10.0.2.105 -p "50001-60000" &

	# 6 threads
#	ip netns exec S ./server -u -H 10.0.1.100,10.0.1.101,10.0.1.102,10.0.1.103,10.0.1.104,10.0.1.105 -P 1001-1500 &
#	ip netns exec C ./client -u -H 10.0.1.100,10.0.1.101,10.0.1.102,10.0.1.103,10.0.1.104,10.0.1.105 -P 1001-1500 \
...
#				    -p 50001-60000 &
	...
	# IPv6
	# 1 thread
#	ip netns exec S ./server -u -H 2000::100 -P 1001-1500 &
#	ip netns exec C ./client -u -H 2000::100 -P 1001-1500 -h 2001::100,2001::101,2001::102,2001::103,2001::104,2001::105 -p 50001-60000 &

	# 6 threads
	ip netns exec S ./server -u -H 2000::100,2000::101,2000::102,2000::103,2000::104,2000::105 -P 1001-1500 &
	ip netns exec C ./client -u -H 2000::100,2000::101,2000::102,2000::103,2000::104,2000::105 -P 1001-1500 -h 2001::100,2001::101,2001::102,2001::103,2001::104,2001::105 -p 50001-60000 &
